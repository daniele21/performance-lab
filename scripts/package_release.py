...
def write_run_instructions(staging: Path, wheel_name: str) -> None:
    (staging / "RUN.md").write_text(
        ...
        "4. Open `http://127.0.0.1:8765`, choose Test a model, then connect your loopback "
        "Local LLM Server or OpenAI-compatible endpoint.\n"
        "5. Stop the foreground Performance Lab process with Ctrl-C.\n\n"
        "If you already maintain a versioned `StarterRunConfig`, `python launch.py --config "
        "<config.json>` remains supported as an advanced/backward-compatible startup path.\n\n"
        "To evaluate regressions in Compare, pass an explicit versioned policy with `python "
        "launch.py --regression-policy <policy.json>`. Repeat `--regression-policy` to expose "
        "multiple policies. When omitted, Compare remains a raw evidence comparison and no "
