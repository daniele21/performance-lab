...
REQUIRED=("README.md","AGENTS.md","CONTRIBUTING.md","SECURITY.md",".editorconfig",".gitignore",".engineering/baseline.json",".engineering/documentation-policy.json",".engineering/commands.json",".engineering/e2e.json",".github/pull_request_template.md",".github/workflows/repository-health.yml","docs/README.md","docs/architecture.md","docs/current-state.md","docs/features/README.md","docs/adr/README.md","docs/workstreams/README.md","scripts/verify_operations.py","scripts/verify_e2e.py","scripts/verify_stage_environment_policy.py","scripts/verify_product_experience.py")
...
def main()->int:
    ...
    except Exception as exc: errors.append(f"invalid baseline.json: {exc}"); b={}
    s=b.get("standard",{})
    ...
    if s.get("revision")!="8aa95d10254846e7d63f4bd5c60d61b18d21060c": errors.append("baseline revision must match canonical 0.9.2")
    if b.get("target_level") not in {"L0","L1","L2"}: errors.append("target_level invalid")
    for name in CORE:
        e=b.get("skills",{}).get(name)
        if not isinstance(e,dict) or not e.get("source_version") or not isinstance(e.get("customized"),bool): errors.append(f"baseline skill metadata invalid: {name}")
