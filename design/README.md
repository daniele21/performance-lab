...
- **Test a model** — guided model -> scenario -> test -> review flow.
...
[`reference/ux-reference-board.svg`](reference/ux-reference-board.svg) is the current canonical UX reference board. It covers:
...
The board is a product reference, not evidence that the UI is shipped. Every displayed metric/control remains conditional on trustworthy backend evidence and supported application contracts.
